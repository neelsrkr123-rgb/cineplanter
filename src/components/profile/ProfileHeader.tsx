...
useEffect(() => {
  ...
  const unsubscribe = onSnapshot(userRef, (docSnap) => {
    if (docSnap.exists()) {
      ...
      setProfile((prev: any) => ({ ...prev, avatar: data.avatar, username: data.username, bio: data.bio, title: data.title, socials: data.socials, followers: data.followers || [], following: data.following || [] }));
      setFollowersCount(data.followers?.length || 0);
      setFollowingCount(data.following?.length || 0);
      setAvatarKey(Date.now());
      if (!editing) {
        setForm((prev: any) => ({ ...prev, avatar: data.avatar || prev.avatar, username: data.username || prev.username, bio: data.bio || prev.bio, title: data.title || prev.title, facebook: data.socials?.facebook || prev.facebook, instagram: data.socials?.instagram || prev.instagram }));
      }
    }
  });
  return () => unsubscribe();
...
const uploadAvatar = async (file: File) => {
  try {
    ...
    setForm((prev: any) => ({ ...prev, avatar: result.url }))
    setAvatarKey(Date.now())
    
    // 🔥 photoURL এর পরিবর্তে avatar ব্যবহার করুন
    await updateUserData({ avatar: result.url })
...
const saveProfile = async () => {
  // 🔥 youtube সহ socials সম্পূর্ণ করুন
