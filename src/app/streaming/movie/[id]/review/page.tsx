...
export default function ReviewPage() {
  ...
  // 🔥 FIXED: Helper function to get user ID from different possible fields
  const getUserId = (): string | null => {
    if (!user) return null
    return user.uid || user.id || null
  }

  // 🔥 FIXED: Helper function to get user name from different possible fields
  const getUserName = (): string => {
    if (!user) return 'Anonymous'
    return user.name || user.username || user.email?.split('@')[0] || 'Anonymous'
  }

  // 🔥 FIXED: Helper function to get user avatar from different possible fields
