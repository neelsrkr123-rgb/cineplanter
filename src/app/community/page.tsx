...
import {
  ...
  increment, where, limit, getDocs, setDoc, FieldValue, Timestamp
...
export default function CommunityMainPage() {
  ...
  const getJoinedPagesData = (): JoinedPage[] => {
    return joinedInterests
      ...
      .filter((page): page is JoinedPage => page !== null);
  };

  const joinedPages = getJoinedPagesData();

  const getDisplayedJoinedPages = (): JoinedPage[] => {
  ...
  // 🔥 FIXED: Real-time posts feed with proper types
  useEffect(() => {
    ...
    const fetchFeed = async () => {
      try {
        let followedUsersPosts: FeedPost[] = [];
        if (followingUsers.length > 0) {
          ...
          followedUsersPosts = usersSnapshot.docs.map(doc => ({
            ...
            userName: doc.data().userName || doc.data().name || "User",
            userPhotoURL: doc.data().userPhotoURL || doc.data().avatar || "",
          } as FeedPost));
        }
        ...
        const postsWithUserData = await Promise.all(uniquePosts.map(async (post) => {
          ...
          return {
            ...
            userName: post.userName || userData?.name || "User",
            userPhotoURL: post.userPhotoURL || userData?.avatar || "",
          };
        }));
  ...
  const createPost = async () => {
    ...
    try {
      ...
      if (selectedImage) {
        try {
          ...
          const result = await uploadToCloudinary(selectedImage, { 
            folder: `community-posts/post_images/${currentUser.uid}`, 
            tags: ['cine-planter', 'community'].concat(postType ? [postType] : [], selectedInterests).filter(Boolean) 
          });
      ...
      if (ytUrl && ytUrl.trim() !== "") { 
        ...
        postData.youtubeUrl = ytUrl.trim(); 
        postData.hasMedia = true; 
        if (videoId) { 
          postData.youtubeVideoId = videoId; 
          postData.youtubeThumbnail = `https://img.youtube.com/vi/${videoId}/hqdefault.jpg`; 
        } 
      }
      
      if (externalLink && externalLink.trim() !== "") { 
        postData.externalLink = externalLink.trim(); 
        postData.hasMedia = true; 
        postData.linkTitle = linkTitle || extractDomainName(externalLink); 
      }
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-slate-200">
      ...
      <Navbar />
       <main className="relative pt-12 pb-10 px-4 md:px-8">
        <div className="max-w-[1800px] mx-auto grid grid-cols-1 lg:grid-cols-[260px_1fr_380px] gap-8 lg:gap-12">
          ...
          <section className="space-y-6 min-w-0 flex flex-col items-center">
            ...
            <div className="w-full max-w-[600px] space-y-4">
              {loading ? (<div className="text-center py-12"><Loader2 className="w-8 h-8 animate-spin text-purple-500 mx-auto" /></div>) : posts.length > 0 ? (posts.map((post) => (
                <PostCard 
                  key={post.id} 
                  post={{
                    ...post,
